...
void jeltisztitas(int zajos[], int csillag[], int tiszta[]) {
    ...
    int zajosHossz=0;
    while(zajos[zajosHossz]!=-1){
        zajosHossz++;
    }

    int csillagHossz=0;
    while(csillag[csillagHossz]!=-1){
        csillagHossz++;
    }

    if(csillagHossz>=zajosHossz){
        for (int i = 0; i <= zajosHossz; ++i) {
            tiszta[i]=zajos[i]-csillag[i];
            if(zajos[i]==-1){
                tiszta[i]=-1;
                break;
            }
    ...
    for (int i = 0; i < zajosHossz/csillagHossz; ++i) {

        for (int j = 0; j < csillagHossz; ++j) {
            tomb[k]=csillag[j];
            k++;
